...
try {
	PreparedStatement smt=con.prepareStatement("select * from cart where delivered='no' ");
...
<tr><td><%= rs.getInt(3)%></td><td>	<%= rs.getString(2)%></td><td><%= rs.getInt(4)%></td><td><i class="fas fa-rupee-sign"></i><%= rs.getInt(5)%></td><td><i class="fas fa-rupee-sign"></i><%= rs.getString(6)%></td>
		    <td><a href="../RemoveAction.jsp?id=<%= rs.getInt(13)%>&page=admin/CancelledOrders"><i class="fas fa-minus-circle"></i>Remove</a></td></tr>
